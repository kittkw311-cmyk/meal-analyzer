...
function buildValueGrid(image,fields=FIELDS,{threshold=false,compact=false}={}){
  ...
  fields.forEach((field,index)=>{
    ...
    const gridCol=compact?0:field.col;
    const gridRow=compact?index:field.row;
    const dx=gridCol*cellW+20;
    const dy=gridRow*cellH+10;
    ctx.drawImage(image,r.sx,r.sy,r.sw,r.sh,dx,dy,cellW-40,cellH-20);
  });
  ...
  return {canvas,cellW,cellH,compact};
}
...
function numericTokens(result){
  ...
  return words.map(word=>{
    ...
    return {text:String(word?.text||''),confidence:Number(word?.confidence||0),x0,y0,x1,y1,cx:(x0+x1)/2,cy:(y0+y1)/2,height:y1-y0};
...
function valueFromCell(cellTokens,field){
  ...
  for(const token of ranked){
    value=normalizeNumber(token.text,field);
    if(value!==null) return value;
  }
  return null;
}
...
function mapGridResult(result,grid,fields=FIELDS){
  const all=numericTokens(result);
  const values={};
  fields.forEach((field,index)=>{
    const col=grid.compact?0:field.col;
    const row=grid.compact?index:field.row;
    const x0=col*grid.cellW,x1=x0+grid.cellW;
    const y0=row*grid.cellH,y1=y0+grid.cellH;
...
function inconsistentPrecisionFields(values){
  const retry=new Set();
  const n=id=>{const value=Number(values[id]);return Number.isFinite(value)?value:null;};
  const muscle=n('input-muscle-val');
  const bone=n('input-bone-val');
  const lean=n('input-leanbody-val');
  if(muscle!==null&&bone!==null&&lean!==null&&Math.abs((muscle+bone)-lean)>=0.009){
    retry.add('input-muscle-val');
    retry.add('input-bone-val');
    retry.add('input-leanbody-val');
  }
  return retry;
}
...
async function recognizeNumbers(Tesseract,image){
  ...
  try{
    ...
    const grid=buildValueGrid(image,FIELDS,{threshold:false,compact:false});
...
function parsePastedText(rawText){
  ...
  const values={};
  for(const[id,regex,index]of rules){
    const raw=text.match(regex)?.[1];
    const normalized=normalizeNumber(raw,FIELDS[index]);
...
async function runBodyOcr(event){
  ...
  try{
    let values=parsePastedText(pasted);
    if(file){
      setLoading(true,'Smart Scale画像を準備中');
      const [Tesseract,image]=await Promise.all([loadTesseract(),loadImage(file)]);
      values={...values,...await recognizeNumbers(Tesseract,image)};
    }
    applyValues(values);
    setLoading(false);
    showEditor();
    const count=FIELDS.filter(f=>fieldValue(f.id)!=='').length;
    showNotice(count===FIELDS.length
      ?'15項目を読み取りました。表示されている数字をそのまま確認して保存してください。'
      :`${count}/15項目を読み取りました。空欄だけ確認・手入力してください。`);
  }catch(error){
    console.error('Body OCR failed:',error);
    setLoading(false);
    showEditor();
    showNotice('読み取りを完了できませんでした。空欄は手入力してください。');
  }finally{
    setLoading(false);
...
function install(){
  ...
  if(analyze){analyze.textContent='体組成を読み取る';analyze.addEventListener('click',runBodyOcr,{capture:true});}
