...
const DISPLAY_APP_VERSION = 'v1.0.33';
...
function hideAiConsultationTab() {
  ...
  const hide = () => {
    ...
    if (tab) { tab.hidden = true; tab.style.display = 'none'; tab.setAttribute('aria-hidden', 'true'); }
...
function installPresetFreeWordSearch() {
  ...
  const install = () => {
    ...
    if (newPresetButton) {
      newPresetButton.hidden = false; newPresetButton.removeAttribute('aria-hidden'); newPresetButton.style.display = 'inline-flex';
      newPresetButton.style.flex = '0 0 44px'; newPresetButton.style.width = '44px'; newPresetButton.style.minWidth = '44px'; newPresetButton.style.height = '44px';
      newPresetButton.style.alignItems = 'center'; newPresetButton.style.justifyContent = 'center'; newPresetButton.style.order = '0'; newPresetButton.style.position = 'relative'; newPresetButton.style.zIndex = '2';
    }
    if (!document.getElementById('preset-freeword-search-style')) {
      const style = document.createElement('style');
      style.id = 'preset-freeword-search-style';
      style.textContent = `
        .presets-toolbar{display:grid!important;grid-template-columns:44px minmax(0,1fr);gap:10px!important;align-items:center!important}
        .preset-freeword-search-wrap{position:relative;display:flex;align-items:center;grid-column:2;grid-row:1;width:100%;min-width:0;max-width:none}
        .preset-freeword-search{box-sizing:border-box;width:100%;min-width:0;height:44px;padding:0 12px 0 38px;border:1px solid var(--design-border,#d7dbe2);border-radius:12px;background:var(--design-card,#fff);color:inherit;font:inherit;outline:none}
        ...
        #presets-manual-toggle{display:inline-flex!important;visibility:visible!important;opacity:1!important;grid-column:1;grid-row:1;box-sizing:border-box;flex:none!important;width:44px!important;min-width:44px!important;height:44px!important;align-items:center;justify-content:center}
        #preset-category-filter{grid-column:1 / -1;grid-row:2;width:100%;min-width:0}
        #preset-sort-select{grid-column:1 / -1;grid-row:3;width:100%;min-width:0}
        @media(min-width:721px){.presets-toolbar{grid-template-columns:44px minmax(220px,1fr) auto auto}.preset-freeword-search-wrap{grid-column:2;grid-row:1}#preset-category-filter{grid-column:3;grid-row:1;width:auto}#preset-sort-select{grid-column:4;grid-row:1;width:auto}}
      `;
      document.head.appendChild(style);
    }
    let noResults = document.getElementById('preset-freeword-no-results');
    if (!noResults) {
      noResults = document.createElement('div'); noResults.id = 'preset-freeword-no-results'; noResults.className = 'preset-freeword-no-results';
...
function installNutritionLabelFirstMealRegistration() {
  ...
  const install = () => {
    ...
    button.addEventListener('click', async event => {
      const cameraInput = document.getElementById('meal-camera-input'); const galleryInput = document.getElementById('meal-gallery-input');
      const imageFile = cameraInput?.files?.[0] || galleryInput?.files?.[0] || null;
      if (!imageFile) return;
      event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation();
      const originalText = button.textContent; button.disabled = true; button.textContent = '栄養表示を読み取り中…';
      try {
        const extractForm = new FormData(); extractForm.append('file', imageFile, imageFile.name || 'meal.jpg');
        ...
        const dateValue = document.getElementById('meal-date-input')?.value || ''; const textInput = document.getElementById('meal-text-input')?.value?.trim() || '';
        const saveForm = new FormData();
        saveForm.append('name', String(extracted.name || textInput || '食事').slice(0, 40)); saveForm.append('calories', String(values[0])); saveForm.append('protein', String(values[1])); saveForm.append('fat', String(values[2])); saveForm.append('carbohydrates', String(values[3]));
        saveForm.append('mealDate', buildMealDateFromDateInput(dateValue)); saveForm.append('mealType', selectedType); saveForm.append('baseAmount', String(Number(extracted.baseAmount) > 0 ? extracted.baseAmount : 1)); saveForm.append('servingUnit', extracted.servingUnit === 'g' ? 'g' : '個'); saveForm.append('image', imageFile, imageFile.name || 'meal.jpg');
        const saveResponse = await fetch('/api/history/preset', { method: 'POST', body: saveForm, cache: 'no-store' });
        const saved = await saveResponse.json().catch(() => ({})); if (!saveResponse.ok) throw new Error(saved.error || 'メニューの登録に失敗しました。');
        button.textContent = '登録しました'; window.setTimeout(() => window.location.reload(), 250);
      } catch (error) {
        console.error('Nutrition-label-first meal registration failed:', error); button.textContent = '読み取り失敗・通常AIで再試行'; button.disabled = false; button.dataset.nutritionLabelFallback = '1'; window.alert(error?.message || '栄養表示の読み取りに失敗しました。');
      } finally { if (button.textContent !== '登録しました' && button.dataset.nutritionLabelFallback !== '1') { button.textContent = originalText; button.disabled = false; } }
    }, true);
  };
